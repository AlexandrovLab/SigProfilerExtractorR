...
sigprofilerextractor <- function(input_type, output, inputdata, refgen = 'GRCh37', genome_build="GRCh37",minsigs = 1, maxsigs = 3,  replicates=5, mtype = c('96,DINUCS,ID'), init="random",exome=F, cpu=-1) {
  ...
  exome=FALSE
  cpu = as.integer(cpu)
  sigpro$sigProfilerExtractor(input_type, output, inputdata, refgen = 'GRCh37',genome_build="GRCh37",startProcess=minsigs, endProcess = maxsigs, totalIterations=replicates, mtype="96", exome=exome, init=init)
  sys$stdout$flush()
